...
open class LineChartRenderer: LineRadarRenderer
{
    ...
    @objc open func drawLinear(context: CGContext, dataSet: LineChartDataSetProtocol)
    {
        ...
        if dataSet.colors.count > 1, !dataSet.isDrawLineWithGradientEnabled
        {
            ...
            for j in _xBounds.dropLast()
            {
                ...
                // --- ADD THIS GAP CHECK ---
                // If x-values differ by more than 1.0, skip drawing
                if (e2.x - e1.x) >= 3600 {
                    continue
                }
        ...
        { // only one color per dataset
          // 1) Get the maximum gap from your new property
            ...
            // We'll collect "segments" in a single CGPath
            var path = CGMutablePath()
            path.move(to: CGPoint(x: CGFloat(firstEntry.x), y: CGFloat(firstEntry.y * phaseY)))

            // We'll track a "previous" entry
            var prevEntry = firstEntry

            // Optional: track "isolated" gap points if you want to replicate Android's `drawDotsList`
            ...
            for x in stride(from: _xBounds.min + 1, through: _xBounds.min + _xBounds.range, by: 1) {
                ...
                if maximumGap > 0.0 && gapSize > maximumGap {
                    ...
                    // If we got consecutive gaps, Android draws a tiny dot for that point
                    // (since there's no line from prev->cur)
                    if justHadGap {
                        gapPoints.append(prevEntry)
                    }
                    justHadGap = true

                    // -> Close the old segment: Move the path to current point so we "break" the line
                    path.move(to: CGPoint(x: CGFloat(curEntry.x),
                                          y: CGFloat(curEntry.y * phaseY)))
                }
                else {
                    // === No gap => draw line from prevEntry to curEntry ===
                    justHadGap = false
        ...
        bounds: XBounds)
    {
        ...
        // We'll break into segments of up to 128 points, same as Android
        ...
        for index in stride(from: startIndex+1, through: endIndex, by: 1) {
            ...
            if breakInData || (index - currentStartIndex >= indexInterval) || (index == endIndex) {
                ...
                if let drawable = dataSet.fill {
                    drawFilledPath(context: context, path: subPath, fill: drawable, fillAlpha: dataSet.fillAlpha)
                } else {
                    drawFilledPath(context: context, path: subPath, fillColor: dataSet.fillColor, fillAlpha: dataSet.fillAlpha)
                }

                // Start a new segment at the break
    ...
    {
        // This mirrors Android's "generateFilledPath".
        let filled = CGMutablePath()
        guard let firstEntry = dataSet.entryForIndex(startIndex) else { return filled }
